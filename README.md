...
- **Generate Mortal**: distributes dots using standard Chronicles of Darkness creation spreads (Attributes 5/4/3, Skills 11/7/4), picks a random name. Respects the preset currently selected in Sheet Configuration to generate a Hunter, Mage, or any other splat directly
...
Characters are saved in your **browser's local storage** with no account or server required. This means:
...
- Clearing browser data will delete your saves, so export important characters regularly
...
Use **Add to Scene** in the sidebar to bring saved characters into play. Characters are shown grouped by folder if folders are set up. Each character you add becomes an independent **instance**, so you can add multiple instances of the same character for groups of identical NPCs.
...
Use **Export** in the library editor to download your supplement as a `supplement.json` file. Use **Import** to merge entries from a file (existing same-name entries are kept; duplicates are skipped). Export regularly, as clearing your browser data will erase your supplement.

---

## File System Storage

Mortals+ can connect folders on your device as an alternative storage location for characters and library data. This is optional. Browser storage continues to work exactly as before for users who don't connect any folders.

**Supported browsers:** Chrome and Edge only. The feature uses the [File System Access API](https://developer.mozilla.org/en-US/docs/Web/API/File_System_API), which is not available in Firefox or Safari. Buttons appear on unsupported browsers but clicking them explains the limitation.

### Why use it

- **More storage**: browser local storage is limited to ~5 MB per origin. A connected folder has no such limit
- **Data safety**: files in a connected folder survive browser data clears, unlike local storage
- **Cloud sync**: point Mortals+ at a OneDrive, Dropbox, or Google Drive folder to share character sheets across devices or with other players in a group

### Three independent folder connections

Each connection is optional and independent:

- **Characters folder** (Character Sheet panel): each saved character is written as a named `.json` file (e.g. `Elise Varro.json`). Characters from the folder and browser storage appear in a single unified list; the folder copy takes priority if both exist under the same name
- **data.json folder** (Data Library panel): point this at a folder containing a custom `data.json` to override the built-in library at startup. Useful for groups maintaining a shared homebrew library. Requires a page reload to take effect after connecting or disconnecting. If the file is malformed or missing, Mortals+ falls back to the built-in library and shows a persistent warning
- **Supplement folder** (Data Library panel): your Library Editor entries are saved as `supplemental.json` in this folder in addition to browser storage. The folder copy is applied on top of the browser supplement at startup, with folder entries winning on name conflicts

### Connecting a folder

Click the relevant **📁 Connect** button in the Character Sheet or Data Library panel. A one-time advisory will explain the write access implications and recommend using a dedicated subfolder (e.g. a folder named `Mortals+` inside your OneDrive). After acknowledging, your browser's native folder picker opens.

> **Use a dedicated folder.** Do not point Mortals+ at a general-purpose folder like Documents or a cloud storage root. Mortals+ will write `.json` files directly into the chosen folder and may overwrite existing files whose names match character names or the reserved filenames `supplemental.json` and `data.json`.

### Sessions and permissions

Folder connections are remembered between sessions using IndexedDB (not browser local storage). The browser requires one click of approval per session to re-grant access to a connected folder. A clickable prompt appears at the bottom of the screen when this is needed. On Chrome 122+, the browser's permission dialog offers an **Allow on every visit** option that eliminates this prompt permanently.

### How saving works

When a folder is connected, every save writes to **both** the folder and browser storage. Browser storage acts as a live fallback: if the folder becomes unavailable your data is still there. Status messages notify you if either write fails.

### Notes

- Changes made directly to files in the connected folder (e.g. via a text editor or synced from another device) are picked up on the next page load, not in real time
- Folder characters do not carry tags or folder-group assignments from browser storage. Those are browser-local metadata, and characters loaded from a folder will appear ungrouped
...
For code contributions, the frontend is split across three files with no build step or external dependencies:

- `index.html`: HTML structure and external resource references
- `style.css`: all styling: colour tokens, themes, layout, responsive breakpoints, print stylesheet
- `app.js`: all application logic: no external libraries or frameworks
- `data.json`: all configuration and content

Run locally using the **Live Server** VS Code extension (`Go Live` button). Do not open `index.html` directly via `file://` as the `fetch('./data.json')` call will be blocked by browser security.
